...
class Sync extends Component
{
  ...
  /**
   * Separate method that prevents syncing if a sync has already occurred or is queued to sync.
   * Called via the isCpRequest. Other sync initiators like the console should always fire
   * @return array
   */
  ...
  {
	  if(!Telemetron::$plugin->settings->syncEnabled){
			$status = [
				'success' => false,
    ...
    $table = rawurlencode(Craft::parseEnv(Telemetron::$plugin->getSettings()->tableName));
    try{
  ...
  {
    $this->airtableClient = new Client(Craft::parseEnv(Telemetron::$plugin->getSettings()->apiKey), Craft::parseEnv(Telemetron::$plugin->getSettings()->baseId));
    return $this->airtableClient;
  }

  /**
   * Check to ensure the sync job is not already in the queue.
   * @return bool
   */
  ...
  /**
   * Tracks successful sync job to cache to prevent syncing more than once per day
   */
